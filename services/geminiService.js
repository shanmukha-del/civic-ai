...
function localGrievanceAnalysis(note, village, mandal, availableDepts = [], preferredLang = 'te-IN') {
  ...
  if (electricityRegex.test(lowerNote) || electricityRegex.test(rawNote)) {
    const dept = availableDepts.find(d => d.name.toLowerCase().includes('light') || d.name.toLowerCase().includes('electric') || d.name.toLowerCase().includes('power')) || { id: 2, name: 'Electricity Board' };
    matchedDepts.push({ id: dept.id, name: dept.name });
  }
  ...
  let detected_issues = uniqueMatchedDepts.map((d, index) => {
    ...
    else if (d.id === 2 || d.id === 8) prob = 'Power Outage & Streetlight Failure';
